...
from product.infrastructure.models import CategoryDoument
    ...
    def handle(self, *args, **options):
        ...
        for category_data in categories_data:
            try:
                ...
                result = CategoryDoument.objects(
                    title=category_data["title"]
                ).update_one(
                    set__description=category_data["description"],
                    upsert=True
                )
                ...
                if result == 1:
                    ...
                    self.stdout.write(
                        self.style.WARNING(f"⚠ No changes for category: {category_data['title']}")
                    )
                    
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f"✗ Error processing category '{category_data['title']}': {str(e)}")
                )
